...
def plotTestData(classificationPoints, fileName):
...
def createGraphFromTwoFunctions(scatterFunction, lineFunction, squaredError):
    ...
    functionOneDataframe = scatterFunction['trainingDataFrame']
    functionOneName = scatterFunction['training_function'].name

    # Second function dataframes and names
    functionTwoDataframe = lineFunction['BestFunction']['idealDataFrame']
    functionTwoName = lineFunction['BestFunction']['BestFunction'].name
...
def classificationGraphPlot(point, idealFunction):
    ...
    if idealFunction is not None:
        functionClassificationDataframe = idealFunction['idealDataFrame']
        # Get string points of Y based on X
        pointString = "({},{})".format(point["x"], round(point["y"], 2))
        title = "point: {} with classification: {}".format(pointString, idealFunction['BestFunction']['BestFunction'].name)
